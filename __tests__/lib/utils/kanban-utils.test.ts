...
describe("findColumnByIdOrJob", () => {
  ...
  it("컬럼 ID로 컬럼을 찾을 수 있다", () => {
    const result = findColumnByIdOrJob(columns, "interest");
    expect(result?.id).toBe("interest");
  ...
  it("존재하지 않는 ID로 찾으면 undefined를 반환한다", () => {
    const result = findColumnByIdOrJob(columns, "nonexistent");
...
describe("reorderJobsInColumn", () => {
  it("같은 컬럼 내에서 Job 순서를 변경할 수 있다 (앞으로 이동)", () => {
    ...
    const result = reorderJobsInColumn(columns, "interest", 3, 1);

    const interestColumn = result.find((col) => col.id === "interest");
    expect(interestColumn?.jobs.map((j) => j.id)).toEqual([3, 1, 2]);
  });

  it("같은 컬럼 내에서 Job 순서를 변경할 수 있다 (뒤로 이동)", () => {
    const columns = createMockColumns();
    const result = reorderJobsInColumn(columns, "interest", 1, 3);
  ...
  it("overId가 컬럼 ID면 맨 끝으로 이동한다", () => {
    const columns = createMockColumns();
    const result = reorderJobsInColumn(columns, "interest", 1, "interest");
  ...
  it("같은 위치로 이동하면 변경 없이 반환한다", () => {
    const columns = createMockColumns();
    const result = reorderJobsInColumn(columns, "interest", 1, 1);
  ...
  it("존재하지 않는 컬럼이면 변경 없이 반환한다", () => {
    const columns = createMockColumns();
    const result = reorderJobsInColumn(columns, "nonexistent", 1, 2);
  ...
  it("다른 컬럼은 영향받지 않는다", () => {
    ...
    const appliedColumn = result.find((col) => col.id === "applied");
...
describe("moveJobBetweenColumns", () => {
  it("Job을 다른 컬럼으로 이동할 수 있다", () => {
    const columns = createMockColumns();
    const result = moveJobBetweenColumns(columns, "interest", "applied", 2, 4);
  ...
  it("빈 컬럼으로 Job을 이동할 수 있다", () => {
    const columns = createMockColumns();
    const result = moveJobBetweenColumns(columns, "interest", "interview", 1, "interview");

    const interestColumn = result.find((col) => col.id === "interest");
    const interviewColumn = result.find((col) => col.id === "interview");
  ...
  it("overId가 컬럼 ID면 맨 끝에 추가한다", () => {
    const columns = createMockColumns();
    const result = moveJobBetweenColumns(columns, "interest", "applied", 1, "applied");
  ...
  it("존재하지 않는 컬럼이면 변경 없이 반환한다", () => {
    const columns = createMockColumns();
    const result = moveJobBetweenColumns(columns, "nonexistent", "applied", 1, 4);
  ...
  it("존재하지 않는 Job이면 변경 없이 반환한다", () => {
    const columns = createMockColumns();
    const result = moveJobBetweenColumns(columns, "interest", "applied", 999, 4);
...
describe("isSameDropPosition", () => {
  it("같은 컬럼, 같은 인덱스면 true를 반환한다", () => {
    expect(isSameDropPosition("interest", "interest", 0, 0)).toBe(true);
    expect(isSameDropPosition("applied", "applied", 2, 2)).toBe(true);
  });

  it("다른 컬럼이면 false를 반환한다", () => {
    expect(isSameDropPosition("interest", "applied", 0, 0)).toBe(false);
  });

  it("같은 컬럼이지만 다른 인덱스면 false를 반환한다", () => {
    expect(isSameDropPosition("interest", "interest", 0, 1)).toBe(false);
  });

  it("originalColumnId가 null이면 false를 반환한다", () => {
    expect(isSameDropPosition(null, "interest", 0, 0)).toBe(false);
  });

  it("originalIndex가 null이면 false를 반환한다", () => {
    expect(isSameDropPosition("interest", "interest", null, 0)).toBe(false);
  });
});
